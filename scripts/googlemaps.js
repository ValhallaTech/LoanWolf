        ...
        navigator.geolocation.getCurrentPosition ( position =>
            {
                ...
                bounds.extend (
                    pos ); //Bounds indicates the size of the search field. Represented either as a rectangle or a radius, measured in meters.  
                ...
                infoWindow.setContent (
                    "Location Found" ); //The string is returned if the geolocation services have succeeded. 
                infoWindow.open ( map );
                map.setCenter ( pos ); //Determines the center the map will default too.

                getNearbyPlaces (
                    pos ); //This will allow the function getNearbyPlaces to retrieve the users geolocation or default location.
            },
            () =>
            {
    ...
    const request = {
        ...
        rankBy:
            google.maps.places.RankBy
                .DISTANCE, //rankBy requires you to specify a location (geolocation or default) and .DISTANCE blocks the user from providing a radius or bounds. The default search field is 5 miles. 
...
function nearbyCallback ( results, status )
{
    ...
    {
        createMarkers (
            results ); //createMarkers will places up to 20 markers on the maps with the given type of search (bank) within 5 miles of the location, either the users geolocation or default location.
    }
}
